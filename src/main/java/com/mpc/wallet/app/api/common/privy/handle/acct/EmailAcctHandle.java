...
import com.mpc.wallet.app.api.common.privy.bo.account.EmailAcctBo;
...
@Service
@Slf4j
public class EmailAcctHandle implements AcctHandle, InitializingBean {
    @Override
    public AccountBo handAcct(String value) {
        EmailAcctBo bo = JSONObject.parseObject(value , EmailAcctBo.class);
        if (Objects.isNull(bo)) {
            log.info("[解析Privy数据]解析邮箱账户返回空.value={}" , value);
            return null;
        }
    ...
    public void afterPropertiesSet( ) throws Exception {
        EXEC_MAP.put(LoginTypeEnum.EMAIL , this);
    }
}